...
void Transform::rotateAroundPoint(const glm::vec3& point, const glm::vec3& angle){
    glm::vec3 delta = m_position - point;
    glm::quat q = glm::quat(angle);
    glm::quat q_inv = glm::inverse(q);
    glm::quat p = glm::quat(0, delta.x, delta.y, delta.z);
    glm::quat p_rot = q * p * q_inv;
    m_position = point + glm::vec3(p_rot.x, p_rot.y, p_rot.z);
